...
def test_normalize_param_value_handles_paths_and_arrays() -> None:
    assert optuna_sweep.normalize_param_value(Path("foo")) == "foo"
    assert optuna_sweep.normalize_param_value(np.int64(3)) == 3
    assert optuna_sweep.normalize_param_value([1, 2, 3]) == "[1, 2, 3]"


def test_infer_param_kind_detects_numeric() -> None:
    series = pd.Series(list(range(20)))
    assert optuna_sweep.infer_param_kind(series) == "numeric"


def test_infer_param_kind_detects_categorical() -> None:
    series = pd.Series(["a", "b", "a", "c"])
    assert optuna_sweep.infer_param_kind(series) == "categorical"
...
def test_interaction_matrix_pivot() -> None:
    ...
    pivot = optuna_sweep.interaction_matrix(
        df,
        param_x="param.a",
        param_y="param.b",
        bins=3,
