"""Mojo-backed collision kernels for candidate generation.

This module mirrors the donor-branch collision prototype: Python stages
contiguous CPU buffers and lazily imports a Python-importable Mojo module that
executes the numeric kernels.
"""
...
def is_mojo_thread_context_supported() -> bool:
    """Return True when Python-imported Mojo kernels may run in this thread.

    Streamlit executes app scripts on a ScriptRunner worker thread. The current
    Python-imported Mojo collision kernels can segfault from that context, while
    the same kernels run correctly in normal CLI/test processes. Keep the
    thread check centralized so callers can use a safe fallback.
    """
...
def point_mesh_distance_mojo_subprocess(
    points: torch.Tensor,
    triangles: torch.Tensor,
    *,
    workers: int | None = None,
) -> torch.Tensor:
    """Run the Mojo point-to-mesh distance kernel in a subprocess."""

    result = run_mojo_subprocess(
        "collision_distance",
        {
            "points": points.detach().to(device="cpu", dtype=torch.float32),
            "triangles": triangles.detach().to(device="cpu", dtype=torch.float32),
            "workers": workers,
        },
    )
    return result["distance"].to(device=points.device, dtype=points.dtype)
...
def clearance_mask_mojo_subprocess(
    points: torch.Tensor,
    triangles: torch.Tensor,
    *,
    min_distance: float,
    ...
    """Run the Mojo clearance kernel in a subprocess and return the keep mask."""
    ...
    return result["keep"].to(device=points.device)
...
def path_collision_mask_mojo_subprocess(
    origin: torch.Tensor,
    targets: torch.Tensor,
    triangles: torch.Tensor,
    *,
    ...
    """Run the Mojo segment-collision kernel in a subprocess."""

    result = run_mojo_subprocess(
        "collision_path",
        {
            "origin": origin.detach().to(device="cpu", dtype=torch.float32),
            "targets": targets.detach().to(device="cpu", dtype=torch.float32),
            "triangles": triangles.detach().to(device="cpu", dtype=torch.float32),
            "workers": workers,
        },
    )
    return result["collide"].to(device=targets.device)
...
__all__ = [
    ...
    "clearance_mask_mojo_subprocess",
    "path_collision_mask_mojo_subprocess",
    "path_collision_mask_mojo",
    "point_mesh_distance_mojo",
    "point_mesh_distance_mojo_subprocess",
]
