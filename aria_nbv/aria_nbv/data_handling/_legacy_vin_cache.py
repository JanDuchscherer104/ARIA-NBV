...
This module implements the legacy VIN snippet cache used by the old oracle
cache path. Remove it once all readers consume the immutable VIN offline store.
...
class VinSnippetCacheWriterConfig(BaseConfig):
...
class VinSnippetCacheDatasetConfig(BaseConfig):
