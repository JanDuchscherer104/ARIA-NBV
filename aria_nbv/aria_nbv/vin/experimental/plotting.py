...
from aria_nbv.utils.plotly_helpers import make_line_trace3d, make_scatter3d
from aria_nbv.utils.plotting import (
    _histogram_bar,
    _histogram_edges,
    _pretty_label,
)
from aria_nbv.utils.vin_plotting import camera_tw_from_p3d, frustum_builder_stub, pose_from_p3d_camera
...
def build_shell_descriptor_figure(
    *,
    u: torch.Tensor,
    f: torch.Tensor,
    r: torch.Tensor,
    ...
    for start, end, label in axes:
        fig.add_trace(make_line_trace3d(start, end, color="#285f82", name=label, width=6))
        fig.add_trace(
            go.Scatter3d(
                x=[end[0]],
                y=[end[1]],
    ...
    fig.add_trace(make_line_trace3d(np.zeros(3), t0, color="#fc5555", name="t = r·u", width=6))
    fig.add_trace(make_line_trace3d(t0, t0 + u0, color="#285f82", name="u (pos dir)", width=6))
    fig.add_trace(make_line_trace3d(t0, t0 + f0, color="#2a9d8f", name="f (forward)", width=6))
...
def build_frustum_samples_figure(
    ...
    *,
    ...
    frustum_pose = pose_from_p3d_camera(p3d_cameras, cam_idx)
    frustum_cam = camera_tw_from_p3d(p3d_cameras, cam_idx)
...
def build_sh_components_figure(
    *,
    lmax: int,
    normalization: str,
    n_az: int = 220,
    ...
    def _unit_dir_from_az_el(*, az: torch.Tensor, el: torch.Tensor) -> torch.Tensor:
        ...
        return vec / (vec.norm(dim=-1, keepdim=True).clamp_min(1e-8))
...
def build_radius_fourier_figure(
    *,
    r_min: float,
    r_max: float,
...
def build_vin_encoding_figures(
    ...
    *,
    ...
    figs: dict[str, go.Figure] = {}
    figs["shell_descriptor"] = build_shell_descriptor_figure(u=u, f=f, r=r)
    if pose_encoder_lff is not None:
        figs.update(build_lff_weight_figures(pose_encoder=pose_encoder_lff))
    if include_legacy_sh:
        figs["sh_components"] = build_sh_components_figure(
            lmax=int(lmax),
            normalization=str(sh_normalization),
        )
        figs["radius_fourier_features"] = build_radius_fourier_figure(
            r_min=r_min,
            r_max=r_max,
            freqs=radius_freqs,
            log1p_counts=log1p_counts,
...
def plot_vin_encodings_from_debug(
    ...
    *,
    ...
    """
    figures = build_vin_encoding_figures(
        debug,
        lmax=lmax,
...
__all__ = [
    ...
    "build_lff_weight_figures",
    "build_lff_response_figures",
    "build_prediction_alignment_figure",
    "build_radius_fourier_figure",
    "build_shell_descriptor_figure",
    "build_sh_components_figure",
