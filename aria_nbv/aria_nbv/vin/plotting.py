...
from ..utils.plotly_helpers import flatten_edges_for_plotly, make_line_trace3d, make_scatter3d
...
from ..utils.vin_plotting import (
    BBOX_EDGE_IDX,
    as_pose_tw,
    camera_tw_from_p3d,
...
def build_voxel_frame_figure(
    ...
    *,
    ...
    pose = pose_first_batch(debug.backbone_out.t_world_voxel)
    extent = debug.backbone_out.voxel_extent
    extent = extent.detach().cpu().numpy()
    if extent.ndim == 2:
        extent = extent[0]

    corners_vox = voxel_corners(extent)
    corners_world = (
        pose.transform(
    ...
    x, y, z = flatten_edges_for_plotly(edges)

    origin = pose.t[0].detach().cpu().numpy()
    axes = pose.R[0].detach().cpu().numpy()
    axes = axes * float(axis_scale)
    axis_traces = [
        make_line_trace3d(origin, origin + axes[:, 0], color="#285f82", name="voxel x"),
        make_line_trace3d(origin, origin + axes[:, 1], color="#2a9d8f", name="voxel y"),
        make_line_trace3d(origin, origin + axes[:, 2], color="#e76f51", name="voxel z"),
    ]
...
def build_geometry_overview_figure(
    ...
    *,
    ...
    ref_pose = pose_first_batch(reference_pose_world_rig)
    if show_reference_axes:
        builder.add_frame_axes(frame=ref_pose, is_rotate_yaw_cw90=False)

    voxel_pose_raw = pose_first_batch(debug.backbone_out.t_world_voxel)
    ...
    if backbone_fields:
        evidence = collect_backbone_evidence_points(
            debug,
        ...
        if evidence:
            ...
            for idx, (name, points_world, values) in enumerate(evidence):
                if points_world.size == 0:
                    continue
                if display_rotate_yaw_cw90:
                    points_world = rotate_points_yaw_cw90(
                        points_world,
    ...
    if candidate_poses_world_cam is not None:
        try:
            centers_rig = centers_rig_from_poses(
                reference_pose_world_rig,
                as_pose_tw(candidate_poses_world_cam),
            ).reshape(-1, 3)
        except Exception:  # pragma: no cover - fallback to debug centers
            centers_rig = debug.candidate_center_rig_m.reshape(-1, 3)
    valid_frac = candidate_valid_fraction(debug).reshape(-1)
...
def build_valid_fraction_figure(debug: Any) -> go.Figure:
    ...
    valid_frac = candidate_valid_fraction(debug).reshape(-1).detach().cpu().numpy()
    ...
    fig.add_trace(
        make_scatter3d(
            centers,
            name=_pretty_label("valid_frac"),
            values=valid_frac,
            colorscale="Viridis",
            colorbar_title=_pretty_label("valid_frac"),
            size=4,
            opacity=0.9,
        ),
    )
...
def build_semidense_projection_figure(
    ...
    *,
    ...
    cam = select_p3d_camera(p3d_cameras.to(points_world.device), int(candidate_index))
    cam_idx = 0
    ...
    frustum_pose = pose_from_p3d_camera(cam, cam_idx) if show_frustum else None
    frustum_cam = camera_tw_from_p3d(cam, cam_idx) if show_frustum else None
    builder = None
    if show_frustum:
        builder = frustum_builder_stub(
            frustum_pose,
    ...
    if pts_np.shape[0] > 0:
        ...
        fig.add_trace(
            make_scatter3d(
                pts_np[~valid],
                name=_pretty_label("out of view"),
                color="#e76f51",
                size=2,
                opacity=0.3,
            ),
...
def build_semidense_projection_feature_maps(
    ...
    *,
    ...
    cam = select_p3d_camera(p3d_cameras.to(xyz.device), int(candidate_index))
...
def build_backbone_evidence_figures(
    ...
    *,
    ...
    for name, tensor in voxel_fields.items():
        ...
        points_world = voxel_indices_to_world_from_cache(
            indices,
            pose=pose,
            extent=extent,
            shape=(d, h, w),
        ...
        fig.add_trace(
            make_scatter3d(
                ...
                colorscale="Viridis",
                colorbar_title=_pretty_label(name),
                size=2,
                opacity=0.7,
            ),
        )
...
def build_se3_closure_figure(
    ...
    *,
    ...
    pose_world_cam = as_pose_tw(candidate_poses_world_cam)
    ...
    pose_world_rig = as_pose_tw(reference_pose_world_rig)
...
def build_voxel_inbounds_figure(
    ...
    *,
    ...
    pose_world_voxel = pose_first_batch(t_world_voxel)
