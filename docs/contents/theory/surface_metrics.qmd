...
- `trimesh` (CPU, simple): `trimesh.proximity.closest_point(mesh, points)` returns witness points `(m,3)`, distances `(m,)`, and face indices `(m,)`. This uses spatial acceleration (e.g., Embree if available) and is robust for large meshes.
...
- ATEK: Adapt `compute_pts_to_mesh_dist()` to also track the argmin face per point and compute the corresponding closest point/barycentric coordinates.
