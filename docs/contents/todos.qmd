...
- [x] Backprojection implementation: `aria_nbv/aria_nbv/rendering/candidate_pointclouds.py`::{`build_candidate_pointclouds`, `_backproject_depths_p3d_batch`} and `aria_nbv/aria_nbv/rendering/unproject.py`::`backproject_depth_with_p3d`.
...
- [x] Implement a VIN model (RRI predictor) in `aria_nbv/aria_nbv/vin` on top of the EFM3D ([EFM3D](../contents/literature/efm3d.qmd)) / EVL backbone (frozen).
  - Input:
    ...
    - For this do a comprehensive review of the EVL architecture to understand how to best integrate the view scorer head on top of it. Consider that we have a [simple script to infer EVL](../../aria_nbv/scripts/run_efm3d_on_ase.py). Another good starting point is our [EFM3D symbol index](../contents/ext-impl/efm3d_symbol_index.qmd) and [EFM3D implementation overview](../contents/ext-impl/efm3d_implementation.qmd).
