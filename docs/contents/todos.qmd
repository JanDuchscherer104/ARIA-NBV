...
- [x] The generated candidate torus should be symmetric to the forward axis of the reference frame in terms of the azimuth dimension, while its elevation band stays gravity aligned. Implemented via `align_to_gravity=True` in `CandidateViewGeneratorConfig` (sampling uses yaw-only, gravity-aligned reference).

- The current options to apply jitter to the view directions, wich are determined by the view
...
The CandidateDepthRenderer yields:
...
- The backprojection is currently implemented in `oracle_rri/oracle_rri/rendering/candidate_pointclouds.py`::{`build_candidate_pointclouds`, `_backproject_depths_p3d_batch`}
...
- Input:
  ...
  - For this do a comprehensive review of the EVL architecture to understand how to best integrate the view scorer head on top of it. Consider that we have a [simple script to ifer the EVL](oracle_rri/scripts/run_efm3d_on_ase.py). Another good starting point is our [EFM3D symbol index](../contents/ext-impl/efm3d_symbol_index.qmd) and [EFM3D implementation overview](../contents/ext-impl/efm3d_implementation.qmd).
