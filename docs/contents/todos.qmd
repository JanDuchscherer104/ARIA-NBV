...
- [x] Implement RRI→ordinal binning:
  ...
  > **Done**: Implemented `oracle_rri/oracle_rri/rri_metrics/rri_binning.py` (`RriOrdinalBinner.fit_from_iterable/transform/save/load`) and wired it into the Lightning CLI (`nbv-fit-binner` / `nbv-train --fit-binner-only`).
...
- [x] Apply CORAL:
  ...
  > **Done**: Implemented `oracle_rri/oracle_rri/rri_metrics/coral.py` using `coral-pytorch` and trained with `coral_loss` in `oracle_rri/oracle_rri/lightning/lit_module.py`.
