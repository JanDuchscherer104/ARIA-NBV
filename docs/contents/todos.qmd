...
- [ ] Euler angles (world frame, deg) show a pitch of zero for all poses. Does this make senese? We wanted the x-axis to gravity aligned without adding any jitter to the view-directions. The world up vector is along +Z. Without any jitter, the x-axes of the camera frusta are horizontal and thuse they should have a roll of zero?
...
- [ ] Implement `oracle_rri/oracle_rri/vin/shell_pose_descriptor.py`:
  - consumes `T_camera_rig` (PoseTW) and computes `r, u, f, scalars`
- [ ] Implement `oracle_rri/oracle_rri/vin/pose_encoder_sh.py`:
...
- [ ] Implement `VinModel` (frozen EVL + trainable head):
  - inputs:
    - `efm: dict[str, Any]`
...
- [ ] Implement `VinPrediction` dataclass (logits, scores, masks)
...
- [ ] Integration test:
  ...
  - run VIN forward on the same subset candidates
...
- [ ] Write `docs/contents/impl/vin_nbv.qmd`:
  - clear I/O contract and tensor shapes
  - EVL feature taps to use
  - exact pose descriptor formulas (r,u,f, scalars) and SH encoding (e3nn)
