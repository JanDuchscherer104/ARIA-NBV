...
candidate-query Transformer over finite candidate sets.
Online discrete $Q_H$, external mesh/oracle-compatible substrates, and
continuous target-then-pose actor-critic policies are lower-priority scaling and
escalation RQs after the finite-candidate evidence, not substitutes for the M5
$Q_H$ result.
See the
...
- a mandatory target-conditioned finite-candidate value model $Q_H$ trained from
  ASE oracle rollout traces, first implemented as a candidate-query Transformer;
...
- a scaling analysis that preserves mesh/oracle target-RRI supervision across
  ASE-wide, external-compatible, and online-discrete settings before considering
  time-permitting continuous policies.

M5 first measures non-myopic headroom:
bounded oracle lookahead selects actions by the same cumulative target-RRI
...
oracle re-evaluation of its selected actions, by endpoint gain and cumulative
target RRI under matched budgets, and by recovered headroom over the learned
...
coverage, broader mesh/oracle-compatible substrates, or online discrete
interaction changes that conclusion before continuous action spaces are
...
first implemented as a candidate-query Transformer, should recover measurable
headroom over myopic learned one-step selection after oracle re-evaluation.
...
cumulative target RRI; endpoint gain evaluates the resulting trajectory:
...
implemented as a candidate-query Transformer:
...
flowchart LR
  ...
  Headroom --> QH["Finite-candidate Q_H<br/>candidate-query Transformer first"]
...
| 2026-08-10 to 2026-08-30 | **M5 - Multi-step target-aware rollouts and Q_H** | Random-valid, oracle-greedy/lookahead, and oracle-scored temperature-softmax rollout data are stable; oracle lookahead headroom is measured with cumulative target RRI; if headroom is positive, finite-candidate $Q_H$ is trained and oracle-evaluated by endpoint gain, cumulative target RRI, and recovered headroom over learned one-step scoring. |
...
- keep thesis scope aligned with [RQ1](questions.qmd#rq1-objective) through
  [RQ6](questions.qmd#rq6-escalation) and the
...
- make bounded oracle-RRI lookahead versus one-step greedy under equal budget
  ...
  selects by cumulative target RRI, while endpoint gain evaluates the trajectory;
...
- require $Q_H$ to beat the learned one-step scorer and one-step model/greedy
  selection on endpoint target gain and report cumulative target RRI under
  matched acquisition and candidate budgets;
- report cumulative target RRI, endpoint target gain, scene RRI, number of
  views, path length, invalid action rate, and runtime;
...
- train $Q_H$ as a finite-candidate value model, first implemented as a
  candidate-query Transformer over scene, target, history, and candidate tokens
  to predict one masked bounded-horizon Q value per candidate; scalar motion or
...
### M6 - Scaling and Online/Continuous Escalation {#roadmap-m6}

**Primary question:** Which scaling or online/continuous escalation is justified
after the mandatory finite-candidate $Q_H$ result?

**Implementation surfaces:** the
[counterfactual RL API](../../reference/aria_nbv.pose_generation.CounterfactualRolloutResult.qmd),
transition datasets, external mesh/oracle-compatible substrate notes, online
discrete $Q_H$ in the ASE mesh/oracle loop, and actor-critic or
continuous-control design notes.

**Exit checks:**

- require M5 $Q_H$ evidence before spending time on quantitative online or
  continuous-control work;
- decide whether the first scale extension is more ASE coverage, an external
  mesh/oracle-compatible substrate, or online discrete $Q_H$ in the current ASE
  mesh/oracle loop;
- preserve target-specific point-mesh supervision for thesis-grade scaling
  ...
  signals;
- document how a future actor would propose continuous target-then-pose actions
  and handle feasibility after online training evidence exists, but do not
  require a quantitative continuous baseline;
...
| Scale | small trusted subset; scene-level held-out subset; full 100 GT-mesh scenes; external mesh/oracle-compatible substrate; online discrete $Q_H$ | confidence intervals, coverage report, oracle throughput, recovered headroom under matched budgets |
