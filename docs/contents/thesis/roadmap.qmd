...
bounded oracle lookahead must improve endpoint target-quality gain over
one-step oracle greedy before learned $Q_H$ gains are interpreted as planning
gains. If that headroom is positive, learned $Q_H$ is evaluated by oracle
re-evaluation of its selected actions and by recovered headroom over the
learned one-step target-conditioned scorer under the same acquisition budget.
If headroom is near zero, the thesis reports that the current candidate
distribution and target-RRI objective are effectively myopic. Online discrete
$Q_H$, full continuous control, Habitat/Isaac simulation, VLM/global planning,
SceneScript-style memory, and real-device deployment remain bridge design or
future work unless the M5 evidence justifies experiment-grade escalation.
...
non-myopic headroom. When it does, a learned finite-horizon candidate-query
value model should recover measurable headroom over myopic learned one-step
selection after oracle re-evaluation.
...
Non-myopic planning is interpreted through oracle-lookahead headroom:
...
flowchart LR
  ...
  Headroom --> QH["Candidate-query Transformer Q_H<br/>masked finite-candidate value learning"]
...
gantt
    ...
    M6 follow-up design                 :m6, 2026-08-31, 2026-09-13
...
| 2026-08-10 to 2026-08-30 | **M5 - Multi-step target-aware rollouts and Q_H** | Random-valid, oracle-greedy/lookahead, and oracle-scored temperature-softmax rollout data are stable; oracle lookahead headroom is measured; if headroom is positive, candidate-query Transformer $Q_H$ is trained and oracle-evaluated by recovered headroom over learned one-step scoring. |
...
- make bounded oracle-RRI lookahead versus one-step greedy under equal budget
  the trusted headroom estimate before interpreting $Q_H$;
...
- require learned $Q_H$ selected actions to be oracle-evaluated under equal
  acquisition budget; if $\Delta_{\mathrm{look}}\approx 0$, report the regime
  as effectively myopic instead of overstating a learned planning failure;
...
| Oracle lookahead has little headroom | The current candidate/objective regime is effectively myopic. | Report $\Delta_{\mathrm{look}}$ honestly, catalog cases where setup actions matter, and do not overclaim $Q_H$ planning gains. |
