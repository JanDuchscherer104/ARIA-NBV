...
target-conditioned candidate-query Transformer $Q_H$ learner over finite
candidate sets.
Online discrete $Q_H$, Habitat, Isaac, and continuous actor-critic policies are
later bridge or future-work substrates, not the thesis-core quantitative path.
See the
...
`docs/typst/thesis/proposal.typ`; the seminar paper is historical implemented
evidence, not the current thesis contract.
...
- stable random-valid, oracle-greedy/lookahead, and oracle-scored
  ...
  diversity evidence;
- a mandatory target-conditioned candidate-query Transformer $Q_H$ trained
  from ASE oracle rollout traces;
...
The quantitative success bar is that learned $Q_H$ beats one-step greedy, the
learned one-step target-conditioned scorer, and one-step model scoring on
cumulative target RRI under the same acquisition budget after oracle evaluation
of selected actions; bounded oracle lookahead is the upper bound. Online
discrete $Q_H$, full continuous control, Habitat/Isaac simulation, VLM/global
planning, SceneScript-style memory, and real-device deployment remain bridge
design or future work unless the M5 evidence justifies experiment-grade
escalation. Hestia-style hierarchy [@Hestia-lu2026], GenNBV-style continuous control
...
budget, a learned finite-horizon candidate-query value model can choose views
whose oracle-evaluated endpoint target-quality gain exceeds myopic learned or
greedy one-step selection under the same candidate and acquisition budgets.
...
are produced as post-$Q_H$ bridge experiments. Invalid candidates are not the
lowest RRI class; invalidity is a hard mask and reason-code contract.
...
| EFM3D / EVL [@EFM3D-straub2024; @EVL-Doc-2025] | Frozen egocentric representation and predicted object support. | Local voxel evidence, OBB predictions, actor-visible target descriptors, support masks. | Treating predicted OBBs as GT or ignoring local extent limits. |
...
s_t^{\mathrm{obs}}
=
\left(P_t^{\mathrm{semi}}, F_t^{\mathrm{EVL}}, O_t^{\mathrm{pred}}, h_t, b_t\right),
$$
...
s_t^{\mathrm{cf0}}
=
\left(F_0^{\mathrm{EVL}}, P_t, z_e, h_t, b_t, Q_t, m_t, \rho_t\right),
$$

where $Q_t=\{q_{t,i}\}_{i=1}^{N_t}$ is the finite candidate table,
$m_{t,i}\in\{0,1\}$ is the hard validity mask, and $\rho_{t,i}$ is the
invalid-reason code. The admissible actions are candidate indices:
...
The V1 target protocol is OBS-SEL / PRED-Q / GT-EVAL:

$$
z_e =
\psi_{\mathrm{obs}}\!\left(O_t^{\mathrm{pred}}, P_t^{\mathrm{semi}}, F_t^{\mathrm{EVL}}, I_{1:t}\right),
\qquad
M_e =
\operatorname{crop}\!\left(M_{\mathrm{GT}}, \operatorname{match}(z_e,O_{\mathrm{GT}})\right).
$$

`z_e` may contain predicted/observed OBB center, orientation, extents, class,
confidence, projected area, semi-dense support, EVL support, and relative pose.
`M_e` is hidden from the actor and used only for labels and endpoint metrics.
...
Let $\Delta_t^e$ be the target-cropped point-mesh oracle error at step $t$:
...
The mandatory planner is a masked candidate-query Transformer:
...
flowchart LR
  ...
  EVL --> TargetSel["OBS-SEL<br/>actor-visible target selection"]
  ...
  RRIOracle --> Rollouts
  Rollouts --> QH["Candidate-query Transformer Q_H<br/>masked finite-candidate value learning"]
  QH --> Eval["Oracle re-evaluation<br/>endpoint target gain, cumulative target RRI, cost"]
  Eval --> Thesis["Thesis figures, ablations, failure cases"]
```

```{mermaid}
gantt
    title ARIA-NBV thesis roadmap
    dateFormat  YYYY-MM-DD
    axisFormat  %d %b
    section Foundation
    M0 scope and proposal freeze        :m0, 2026-04-29, 2026-05-10
    M1 data cache oracle correctness    :m1, 2026-05-11, 2026-05-31
    M2 VIN baseline and scale gates     :m2, 2026-06-01, 2026-06-21
    section Target and Planning Core
    M3 entity target RRI                :m3, 2026-06-22, 2026-07-12
    M4 target conditioned VIN           :m4, 2026-07-13, 2026-08-09
    M5 rollouts and Q_H                 :crit, m5, 2026-08-10, 2026-08-30
    section Bridge and Freeze
    M6 continuous and online bridge     :m6, 2026-08-31, 2026-09-13
...
| 2026-08-10 to 2026-08-30 | **M5 - Multi-step target-aware rollouts and Q_H** | Random-valid, oracle-greedy/lookahead, and oracle-scored temperature-softmax rollout data are stable; candidate-query Transformer $Q_H$ is trained and oracle-evaluated; $Q_H$ beats one-step greedy/model scoring or produces a documented blocker under the thesis success bar. |
| 2026-08-31 to 2026-09-13 | **M6 - Actor-critic and continuous bridge** | Deliver the bridge design from fitted finite-candidate $Q_H$ toward online discrete $Q_H$, IQL, actor-critic, and simulator-backed control; no quantitative continuous-control baseline is required for thesis success. |
...
- freeze the compact advisor proposal before M1 scale-up: ARIA-NBV name and
  ...
  finite-candidate candidate-query Transformer $Q_H$, a compact timeline, and
...
- render the approximately 2.5 content-page proposal contract with
  `make proposal-pdf` and track the generated proposal PDF once the
  advisor-facing text is frozen;
- align proposal, roadmap, and questions on the five-role boundary:
  implemented substrate; prerequisite/evidence protocol; hard quantitative
  thesis core; mandatory bridge design; and future-work extensions;
...
- keep thesis scope aligned with [RQ1](questions.qmd#rq1-objective) through
  [RQ5](questions.qmd#rq5-bridge) and the
...
- compare random-valid, deterministic one-step oracle greedy, the learned
  ...
  oracle-scored temperature-softmax, and candidate-query Transformer $Q_H$
...
- train $Q_H$ as a candidate-query Transformer over scene, target, history, and
  candidate tokens to predict one masked bounded-horizon Q value per candidate;
  scalar motion or rule penalties are extensions;
- require hard validity masks and explicit invalid reason codes in Q_H data;
- require learned $Q_H$ selected actions to be oracle-evaluated and to beat the
  learned one-step target-conditioned scorer plus one-step greedy/model scoring
  on cumulative target RRI under equal acquisition budget, with oracle lookahead
...
transition datasets, online discrete $Q_H$ in the ASE mesh/oracle loop, IQL as
a second offline-RL ablation, and actor-critic or continuous-control design
notes.

**Exit checks:**

- require M5 Q_H evidence before spending time on quantitative IQL,
  actor-critic, or continuous-control work;
- document online discrete $Q_H$ as the first bridge step because it can reuse
  the current finite candidate/action contract and ASE mesh/oracle loop;
...
| Objective | scene RRI; target RRI; target RRI plus cost; log-gain ablation | endpoint target gain, cumulative target RRI, quality-cost curves |
| Planner | random-valid; learned one-step scorer; one-step oracle greedy; bounded oracle lookahead; $Q_H$ | oracle-evaluated cumulative target RRI under equal budget |
...
| Ordinal/scalar loss | CORAL; balanced CORAL; focal threshold; auxiliary Huber | calibration, Spearman correlation, top-k hit, confusion structure |
| State richness | CF0 geometry-only; selected synthetic observations; optional directional observability | planning improvement, support gap, overfitting gap |
| Scale | small trusted subset; scene-level held-out subset; full 100 GT-mesh scenes | confidence intervals and coverage report |
...
| Candidate valid fraction is low | $Q_H$ learns feasibility artifacts instead of utility. | Mixed sampler tuning, reason codes, minimum valid-count gates, candidate provenance diagnostics. |
| Greedy oracle dominates all horizons | Multi-step improvement may be scientifically weak. | Identify occlusion/setup-action cases; report when target RRI is approximately myopic. |
...
| $Q_H$ fails to beat myopic controls | The hard planning claim is falsified or blocked. | Report the failing gate and preserve a defensible target-aware oracle, one-step scorer, and rollout-data study; do not replace it with unvalidated continuous RL. |
