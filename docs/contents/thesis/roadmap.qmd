...
The September deliverable is a reproducible target-aware {{< gls next-best-view >}} stack:
...
|---|---|---|
| 2026-04-29 to 2026-05-10 | **M0 - Scope, repo, docs foundation** | Dirty worktree classified; available partial VIN offline store smoke path understood; Rerun inspector smoke command documented; KG-friendly doc and docstring rules documented; proposal-freeze and bibliography-audit work tracked as advisor deliverables. |
| 2026-05-11 to 2026-05-31 | **M1 - Data, cache, oracle correctness** | Offline store and RRI contracts stable; M1 contract report records store/split/frame/CW90/candidate-alignment/Rerun-smoke evidence; oracle throughput measured. |
...
| 2026-08-10 to 2026-08-30 | **M5 - Multi-step target-aware rollouts** | Deterministic oracle-greedy/lookahead comparison trusted first; stochastic/beam and model-scored rollouts added only after that baseline is reportable. |
| 2026-08-31 to 2026-09-13 | **M6 - Value/RL gate** | If deterministic rollout and VIN evidence are strong, add masked discrete Q or close-greedy RL; otherwise deepen rollout/entity ablations. |
...
- explicit frame semantics for candidate poses, rig poses, and display-only
  CW90 corrections;
- one saved or referenced Rerun smoke recording path when available, without
  committing generated `.rrd` artifacts;
...
- GT-OBB cropped target RRI on a small trusted subset;
...
- compare random-valid, deterministic one-step oracle greedy, and deterministic
  bounded oracle lookahead before adding stochastic/beam or model-scored
  rollout variants;
- make bounded oracle-RRI lookahead versus one-step greedy under equal budget
  the first non-myopic comparison;
- report cumulative target RRI, scene RRI, number of views, path length, invalid
  action rate, and runtime;
- use an episode-normalized additive or log-improvement reward for multi-step
  return while preserving current one-step RRI labels;
...
- require the deterministic M5 oracle-greedy/lookahead comparison to be
  reportable before adding stochastic/Gumbel, VIN-surrogate, value, Q, or return
  heads;
...
- require held-out ranking, oracle-evaluated VIN-selected rollouts, calibration
  and stage-shift analysis, and Rerun failure visualization before adding value,
  Q, or return heads;
