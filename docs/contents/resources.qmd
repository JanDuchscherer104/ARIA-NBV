...
## Oracle Backend Setups

- CUDA workstation profile (`pytorch3d_cuda`): `uv sync --extra dev --extra pytorch3d`
- Apple Silicon profile (`apple_mps_mojo`): `uv sync --extra dev` plus the local Mojo setup described in the Apple-Silicon backend notes
- The CUDA profile is the default even on Apple machines; select the Apple profile explicitly when running the Mojo path.
