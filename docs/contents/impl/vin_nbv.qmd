...
## D) Legacy encoding visualizations

The old illustrative SH/Fourier plots were removed during the thesis cleanup because they reflected a legacy
encoder configuration rather than the current VIN stack. If we want this section back, regenerate figures from the
current implementation instead of restoring the outdated assets.
...
4. **Binning diagnostics (plots).** The script `oracle_rri/scripts/plot_vin_binning.py`
   generates the RRI histogram with quantile edges and a dedicated edge plot (Seaborn). The legacy rendered
   examples were removed during cleanup; regenerate them from the current script if we still want them embedded here.
