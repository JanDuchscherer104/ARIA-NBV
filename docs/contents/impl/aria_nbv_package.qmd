...
- **Active perception/vision**: NBV is an instance of active perception—moving the sensor to harvest more informative observations; active vision systems explicitly reorient cameras to reduce occlusion and improve depth estimates. citeturn1search12turn1search13
- **Point clouds as state**: Our reconstruction state \(P_t\) is a point cloud—an unordered set of 3D samples that approximates scene geometry. citeturn1search0
- **Chamfer vs. Hausdorff**: The Chamfer distance we use for RRI is a smooth, averaged variant of the symmetric Hausdorff distance between two point sets, trading strict worst-case guarantees for robustness to outliers. citeturn0search12
