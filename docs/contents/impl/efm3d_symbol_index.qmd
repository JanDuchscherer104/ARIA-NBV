...
EFM3D is the foundation for our ASE-based NBV research: it standardises Aria sensor snippets, lifts Dino features into volumetric grids, fuses predictions through time, and evaluates reconstruction quality. This page catalogues every symbol from the vendored `external/efm3d` tree that we need, grounding each entry in the relevant theory (see also `rri_theory.qmd`, `oracle_rri_impl.qmd`, and `atek_implementation.qmd`). We emphasise tensor shapes, coordinate conventions, and provide quick usage examples for the key types.
...
| `ARIA_POSE_T_WORLD_RIG` | `PoseTW` `[B, T, 12]` | Transform rig→world. |
...
- **Role**: streams WebDataset shards into ARIA-structured batches.
- **Usage**:

```python
from efm3d.dataset.wds_dataset import WdsStreamDataset

stream = WdsStreamDataset(urls="/data/ase_eval/shards/train-{00000..00099}.tar")
first = next(iter(stream))
rgb = first["rgb/img"]  # torch.float32 [T, 3, 1408, 1408]
```

## `EfmModelAdaptor` (`dataset/efm_model_adaptor.py`)

- **Role**: normalises snippets (origin shift, gravity alignment), selects modalities, and collates batches for EVL.
- **Usage**:

```python
from efm3d.dataset.efm_model_adaptor import load_atek_wds_dataset_as_efm

loader = load_atek_wds_dataset_as_efm(
    input_json_path=".data/aria_download_urls/AriaSyntheticEnvironment_ATEK_download_urls.json",
    config_name="efm_eval",
    batch_size=1,
)

batch = next(iter(loader))
origin_pose = batch["snippet/t_world_snippet"].to_matrix().view(-1, 4, 4)
```
