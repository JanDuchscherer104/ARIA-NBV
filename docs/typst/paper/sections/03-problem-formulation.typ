...
frames and poses. Let $#sym_points _t$ be the current reconstruction point
set at step $t$, and let $#sym_mesh$ denote the ground-truth surface mesh
for the scene. At each step we sample a set of $N$ candidate camera
poses $(q_i)_{i=1}^N in "SE"(3)$ (with optional roll constraints), render
candidate point sets $#sym_points _(q_i)$, and select the view that
maximizes the expected improvement in reconstruction quality.

== Chamfer distance and RRI

We measure reconstruction quality using the symmetric Chamfer distance between a
point set $#sym_points$ and a mesh surface $#sym_mesh$. We sample
points from the mesh surface
and compute the bidirectional distance
...
where $n_P$ and $n_M$ denote the point counts. The Relative
Reconstruction Improvement for candidate $q$ is then
