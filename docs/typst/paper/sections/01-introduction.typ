...
features, and semidense projection statistics. The model incorporates
pose-conditioned global pooling and frustum-aware attention to capture
view-dependent evidence, while remaining small enough to train with limited
oracle labels.
...
- We introduce a VIN v2 architecture that combines EVL voxel features, pose
  encodings, semidense projection statistics, and frustum-aware attention for
  candidate-dependent scoring.
