...
// Progress update
#section-slide(title: [Project Progress], subtitle: [Implemented tooling & sampling pipeline])[
  #text(size: 16pt)[Short recap of the concrete features we shipped for ASE/ATEK handling and NBV sampling.]
]

#slide(title: [Unified ASE Downloader CLI])[
  #color-block(title: [What it does])[
    - Single entrypoint downloads GT meshes + ATEK shards with consistent scene/snippet filtering.
    - SHA1 validation + ZIP extraction for meshes; writes filtered ATEK JSON to call `download_atek_wds_sequences`.
    - Respects `PathConfig` roots; optional mesh or ATEK skip, mesh overwrite, and scene preference logic.
  ]
...
#slide(title: [EFM Dataset Wrapper & Typed Views])[
  ...
  #color-block(title: [Typed views])[
    - `EfmCameraView`, `EfmTrajectoryView`, `EfmPointsView`, `EfmGTView`, `EfmObbView` expose shapes/dtypes and lightweight `.to(...)` moves.
    - `EfmSnippetView` bundles cameras, trajectory, semi-dense points, OBBs, GT, and optional mesh with world-frame helpers and rich `__repr__`.
  ]
]

#slide(title: [Candidate View Generator])[
  #color-block(title: [Sampling])[
    - Shell sampling around latest `PoseTW` with area-uniform or forward-Gaussian elevation; radii in `[min_radius, max_radius]`.
    - Cameras oriented via `view_axes_from_points` to look at the last pose (RDF, VIO world frame).
  ]
  ...
  #color-block(title: [Outputs])[
    - Returns `PoseTW` batch, per-rule masks, valid-mask, and raw shell poses for visualisation; resamples up to `max_resamples` to fill `num_samples`.
  ]
]

#slide(title: [Rule: Shell Sampling])[
  #grid(columns: (1fr, 1fr), gutter: 1cm,
    [
      #color-block(title: [Formulation])[
        $
          r ~ cal(U)(r_{min}, r_{max}),\
          phi ~ cal(U)(0, 2pi)~text("or")~cal(U)(0, pi),\
          u ~ cal(U)(sin theta_{min}, sin theta_{max}),\
          theta = arcsin(u),\
          vec(d)_i = (cos theta cos phi,\; sin theta,\; cos theta sin phi),\
          vec(p)_i = vec(t)_L + R_L (r_i vec(d)_i),\
          T_i \in 	ext("SE(3)").
        $
        Forward-Gaussian: $ theta ~ cal(N)(mu, sigma) $ clipped to $[theta_{min}, theta_{max}]$.
      ]
    ],
    [
...
#slide(title: [Rule: Min Distance])[
  #grid(columns: (1fr, 1fr), gutter: 1cm,
    [
      #color-block(title: [Criterion])[
        $ d_{text("mesh")}(bold(p)_i) > d_{min} $
        $ d_{text("mesh")}(vec(p)_i) > d_{min} $
    ],
    [
      #color-block(title: [Intuition])[
        - Discards viewpoints starting inside / too near GT geometry.\\
        - Uses trimesh `ProximityQuery` signed distances.
      ]
    ],
  )
]
...
#slide(title: [Rule: Path Collision])[
  #grid(columns: (1fr, 1fr), gutter: 1cm,
    [
      #color-block(title: [Criterion])[
        $ bold(r)_i(t) = bold(o) + t\, hat(bold(d)_i),\; t \in [0,\; ||bold(p)_i - bold(o)||] $ \\
        Reject if `intersects_any` hits the mesh.
      ]
    ],
    [
      #color-block(title: [Intuition])[
        - Enforces straight-line, collision-free translation to candidate.\
        - Prefers PyEmbree; falls back to trimesh ray engine.
      ]
    ],
  )
]
#slide(title: [Rule: Free-Space AABB])[
  #grid(columns: (1fr, 1fr), gutter: 1cm,
    [
      #color-block(title: [Criterion])[
        $ x_{min} <= x_i <= x_{max},\; y_{min} <= y_i <= y_{max},\; z_{min} <= z_i <= z_{max}. $
      ]
    ],
    [
      #color-block(title: [Intuition])[
        - Restricts sampling to occupancy bounds from mesh or semidense volume.\
        - Avoids leaking candidates outside the current room/observed region.
      ]
    ],
  )
]
...
#slide(title: [Downloader CLI Usage])[
  #color-block(title: [List scenes with meshes])[
    ```bash
    python -m oracle_rri.data.downloader list --n=8
    ```
  ]
  #color-block(title: [Download meshes + ATEK snippets])[
    ```bash
    python -m oracle_rri.data.downloader download \
      --n_scenes=5 --max_snippets=2 --c=efm_eval \
      --output-dir=/data/ase_atek
    ```
    Flags: `--skip_meshes`, `--skip_atek`, `--prefer_scenes_with_max_snippets`, `--overwrite`.
  ]
]

#slide(title: [Dataset & Typed Views Usage])[
  #color-block(title: [Python snippet])[
    ```python
    ...
    rgb = sample.camera_rgb  # EfmCameraView
    traj = sample.trajectory # EfmTrajectoryView
    sem = sample.semidense   # EfmPointsView
    print(sample.has_mesh, sample.mesh is not None)
    ```
  ]
  #color-block(title: [What you get])[
    - Cameras: `images`, `calib`, `time_ns`, `frame_ids` with `.to(device)`
    - Trajectory: `t_world_rig`, `final_pose()`
    - Semidense: `points_world`, `dist_std`, `lengths`, `volume_min/max`
    - GT/OBBs: `gt.timestamps`, `obbs.obbs`
  ]
]

#slide(title: [Rich Sample Summary Utility])[
  #color-block(title: [rich_summary])[
    - Pretty `rich.tree` view over flattened samples with shapes/stats; optional truncation of GT or semidense keys.
    - `build_nested` reconstructs hierarchical dict + flat-key lookup for debugging dataset fields.
    - Helpful for sanity-checking joined ATEK snippets, meshes, and typed views before downstream RRI experiments.
  ]
]
