...
The reinforcement-learning literature enters the thesis as a finite-candidate planning scaffold, not as a commitment to immediate actor-critic control. Trajectory Transformer shows that offline control can be decoded as a sequence problem with beam search, and Gumbel-Top-k provides a stochastic top-k mechanism for sampling diverse high-scoring sequences without replacement @TrajectoryTransformer-janner2021 @GumbelTopK-kool2019. Double DQN supplies the first mandatory learned value method for ARIA-NBV: a masked fitted Double-Q / $Q_H$ model over finite candidate sets, trained from ASE oracle rollout data @DoubleDQN-vanHasselt2015. IQL, deep energy-based policies, PPO, and SAC remain gated follow-up or bridge references because they expose support, reward-loop, and simulator assumptions that the thesis core should not hide @IQL-kostrikov2021 @DeepEnergyPolicies-haarnoja2017 @PPO-schulman2017 @SAC-haarnoja2018. ARIA-NBV therefore treats `ArgTopK -> ArgTop1_1 -> ... -> ArgTop1_H -> Q_H` as the thesis-grade non-myopic ladder before optional actor-critic or continuous-control work.
...
#figure(
  table(
    ...
    [Begin with deterministic bounded oracle rollout, train masked fitted Double-Q / $Q_H$ after rollout data are trusted, and gate continuous RL behind evidence.],
  ),
