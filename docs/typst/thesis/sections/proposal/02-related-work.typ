...
The reinforcement-learning literature enters the thesis as a planning scaffold, not as a commitment to immediate actor-critic control. Trajectory Transformer shows that offline control can be decoded as a sequence problem with beam search, and Gumbel-Top-k provides a stochastic top-k mechanism for sampling diverse high-scoring sequences without replacement @TrajectoryTransformer-janner2021 @GumbelTopK-kool2019. Double DQN, IQL, deep energy-based policies, and SAC explain later value-learning or stochastic-policy options, but they also expose the evidence burden: value estimates can be overoptimistic, offline policies must respect dataset support, and maximum-entropy control needs a reliable reward surface @DoubleDQN-vanHasselt2015 @IQL-kostrikov2021 @DeepEnergyPolicies-haarnoja2017 @SAC-haarnoja2018. ARIA-NBV therefore treats `ArgTopK -> ArgTop1_1 -> ... -> ArgTop1_H` as the first thesis-grade non-myopic ladder before adding learned value heads.
...
#figure(
  table(
    ...
    [Begin with deterministic bounded oracle rollout and add RL only behind evidence gates.],
  ),
