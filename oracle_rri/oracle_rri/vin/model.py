...
1. **Learnable Fourier pose encoding** (candidate pose in reference frame)
   ...
   and encoded by ``LearnableFourierFeatures`` (LFF), which learns a projection
   matrix and maps sinusoidal features through a small MLP.
...
class VinModelConfig(BaseConfig["VinModel"]):
    ...
    scene_field_channels: list[
        Literal["occ_pr", "occ_input", "counts_norm", "observed", "unknown", "new_surface_prior", "free_input"]
    ] = Field(
    ...
    @classmethod
    def _validate_pose_encoder_lff(cls, value: LearnableFourierFeaturesConfig) -> LearnableFourierFeaturesConfig:
        """Ensure the LFF input dimensionality matches the pose vector definition."""
        if value.input_dim != 8:
            raise ValueError("pose_encoder_lff.input_dim must be 8 for [u, f, r, s] pose vectors.")
        return value
...
class VinModel(nn.Module):
    ...
    - **Pose encoding** via learnable Fourier features over the shell descriptor
      vector ``[u, f, r, s]``.
    ...
    def _forward_impl(
        ...
        voxel_pose_enc = self.pose_encoder_lff.forward(voxel_vec)
        ...
        if self.use_global_pool:
            # TODO: don't just use a global mean pool. we need to retain more information about the scene!
        ...
        # TODO: most candidates become invalid as the EVL voxel grid is quite small (4m cube). Hence we cannot mask all
        # the candidates that are masked out by _candidate_valid_from_token based on min_valid_frac.
