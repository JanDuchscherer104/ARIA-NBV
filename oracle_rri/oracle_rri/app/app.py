...
from .ui import candidate_config_ui, dataset_config_ui, oracle_config_ui, renderer_config_ui
...
class NbvStreamlitApp:
    ...
    def _render(self, console: Console) -> None:  # pragma: no cover - UI code
        st.set_page_config(page_title="NBV Explorer (refactored)", layout="wide")
        ...
        def _page_data() -> None:
            with st.sidebar.form("data_form"):
                ...
                dataset_cfg = dataset_config_ui(
                    st.sidebar, verbosity=console.verbosity, is_debug=dataset_cfg_prev.is_debug
                )
                sample_idx = st.number_input("Sample index", min_value=0, value=int(state.sample_idx), step=1)
        ...
        def _page_candidates() -> None:
            ...
            state.labeler_cfg = state.labeler_cfg.model_copy(update={"generator": cand_cfg})
        ...
        def _page_renders() -> None:
            with st.sidebar.form("depth_form"):
                ...
                stride = st.slider("Backprojection stride", 1, 32, stride_prev, step=1, key="depth_stride")
        ...
        def _page_rri() -> None:
            ...
            labeler_cfg = state.labeler_cfg.model_copy(
                update={"oracle": oracle_cfg, "backprojection_stride": int(stride), "output_device": "cpu"}
            )
...
NbvStreamlitAppConfig.model_rebuild(_types_namespace={"NbvStreamlitApp": NbvStreamlitApp})
