...
---
::: {.callout-warning}
This page is retained as renderable backlog history only. Current actionable
work is tracked in `.agents/issues.toml`, `.agents/todos.toml`, and
`.agents/refactors.toml`; current thesis milestones are in
[roadmap](../thesis/roadmap.qmd).
:::
...
- [ ] Shuffle candidates and GT RRI values before each training step to avoid any ordering bias.

### Lightning

- [ ] Add `torchmetrics.Accuracy` to the LitModule (aria_nbv/aria_nbv/lightning/lit_module.py)
...
- [ ] Provide theory and conceptual brakedown of `aria_nbv/aria_nbv/rri_metrics/metrics.py`::`chamfer_point_mesh_batched` in `docs/typst/slides/slides_3.typ`. The current contents of the file serves as a template - it was copied from `slides_2.typ` which was our second presentation. Maintain the general style. And use `get-library-docs "/websites/typst_app"` to retrieve typst documentation for the different commands that you might need.
  - [ ] Provide formulas for the different loss components that we use in ``aria_nbv/aria_nbv/rri_metrics/metrics.py``::`chamfer_point_mesh_batched`.

# Candidate View Generation & Sampling

## Current status

- Candidate center sampling (azimuth/elevation/radius caps) and rule-based pruning are implemented in `aria_nbv/aria_nbv/pose_generation/`.
...
- [ ] `CandidateSamplingResult.reference_pose` is not gravity-aligned, even for `align_to_gravity=True`. Added a new field `sampling_pose` that is gravity-aligned when enabled. Downstream tasks and offline batches did not use the gravity aligned reference.
  - **Downstream effects (concise)**:
    - **Rendering**: unaffected (depths/point clouds use `poses_world_cam`; `reference_pose` is metadata only).
    - **VIN pose encoding**: uses `reference_pose_world_rig` (physical rig frame) → relative pose frame includes a fixed roll/pitch offset when `align_to_gravity=True`, so pose encodings are *not* gravity-stable.
    - **VINv3 frame consistency**: `model_v3.py` consumes `candidate_poses_world_cam`, `reference_pose_world_rig`, and `p3d_cameras`. As long as all three are in the **same physical rig frame**, learning is still possible. A **major issue** only arises if we mix frames (e.g., gravity‑aligned ref pose with unaligned `p3d_cameras` or candidate poses).
      - **Offline cache check**: `candidate_poses_world_cam` and `p3d_cameras` match exactly (max abs R diff = 0, max abs T diff ~1e-6).
      - **apply_cw90_correction pitfall**: undoing `rotate_yaw_cw90` on poses only (current behavior when `apply_cw90_correction=True`) breaks that alignment (max abs R diff ~1.25, max abs T diff ~12 for a cached sample), so semidense projection features are in a different camera frame than the pose/global context.
      - **Actionable**: for cached data, keep `apply_cw90_correction=False` or rotate `p3d_cameras` (and `reference_pose_world_rig`) in lockstep with candidate poses.
    - **Diagnostics**: “reference‑frame” pose histograms reflect the physical rig frame, not the gravity‑aligned sampling frame.
    - **Offline cache**: stores only `reference_pose` (no `sampling_pose`), so cached datasets cannot be retrofitted without recompute (not possible right now).
    - **Practical stance (no recompute)**: treat `align_to_gravity` as *sampling convenience* only; keep VIN inputs in the physical rig frame to avoid pose/camera mismatches.
...
- Implement `CandidateViewGenerator` and auxiliary modules in `aria_nbv/aria_nbv/pose_generation` that generate `N` candidate SE(3) poses (roll-free base orientation) around the latest camera position in the trajectory. Must generate poses of type `PoseTW` (`external/efm3d/efm3d/aria/pose.py`).
...
- We now implement candidate rendering + backprojection fully in `aria_nbv/aria_nbv/rendering`:
...
- [x] Backprojection implementation: `aria_nbv/aria_nbv/rendering/candidate_pointclouds.py`::`build_candidate_pointclouds` and `aria_nbv/aria_nbv/rendering/unproject.py`::{`backproject_depths_p3d_batch`, `backproject_depth_with_p3d`}.
...
- Entrypoint: `uv run nbv-st` → `aria_nbv/aria_nbv/streamlit_app.py`
- Package: `aria_nbv/aria_nbv/app/`
...
- Shared compute pipeline: `OracleRriLabeler` (`aria_nbv/aria_nbv/pipelines/oracle_rri_labeler.py`) used by the RRI
...
- **What was wrong:** The legacy app in `aria_nbv/aria_nbv/dashboard/` mixed UI, caching, and compute; this made the
  data-flow hard to reason about and prevented reusing the exact same code path for online label generation.

- **Fix:** Rebuilt the app in `aria_nbv/aria_nbv/app/` with explicit separation of concerns:
  - orchestration in `PipelineController`,
  - compute in `OracleRriLabeler` (`aria_nbv/aria_nbv/pipelines/oracle_rri_labeler.py`),
...
- **Fix:** Added explicit, typed stage caches keyed by `(cfg_sig, sample_key, ...)` and a `force=True` path for refresh
  buttons in `PipelineController` (`aria_nbv/aria_nbv/app/controller.py`).
...
- **Fix:**
  ...
  - enabled `tool.uv.preview = true` in `aria_nbv/pyproject.toml`.
...
- [ ] Cache `VinOracleBatch`:
  ...
  > **Done**: Added `aria_nbv/aria_nbv/data/offline_cache.py` with cache writer/reader, config snapshots (sans tar_urls/scene_to_mesh), and `OracleRriCacheDataset` integration in `VinDataModule`.
...
Goal: Learn a **View Introspection Network (VIN)** that predicts per-candidate **RRI** from (i) frozen **EVL** scene features and (ii) a **shell-aware pose descriptor** computed in the **reference frame**. VIN is trained standalone using oracle labels from `aria_nbv/aria_nbv/pipelines/oracle_rri_labeler.py`.
...
- Label pipeline implementation: `aria_nbv/aria_nbv/pipelines/oracle_rri_labeler.py`
...
- [x] Implement a VIN model (RRI predictor) in `aria_nbv/aria_nbv/vin` on top of the EFM3D ([EFM3D](../literature/efm3d.qmd)) / EVL backbone (frozen).
  - Input:
    ...
    - loss and training objective same as in [VIN-NBV](../literature/vin_nbv.qmd)
    - start with an implementation proposal of the model architecture and training procedure in `docs/contents/impl/vin_nbv.qmd`.
    - For this do a comprehensive review of the EVL architecture to understand how to best integrate the view scorer head on top of it. Consider that we have a [simple script to infer EVL](../../../aria_nbv/scripts/run_efm3d_on_ase.py). Another good starting point is our [EFM3D symbol index](../ext-impl/efm3d_symbol_index.qmd) and [EFM3D implementation overview](../ext-impl/efm3d_implementation.qmd).

    > **Done**: Implemented VIN package code in `aria_nbv/aria_nbv/vin/` (frozen EVL + CORAL head), shell+SH pose encoding via `e3nn` **plus 1D Fourier features for the radius**, and a Lightning-based training pipeline (`aria_nbv/aria_nbv/lightning/` + `nbv-train` / `nbv-fit-binner`) that uses `OracleRriLabeler`. Documented the architecture + training procedure in `docs/contents/impl/vin_nbv.qmd`.
...
Processing strategy (current implementation in `aria_nbv/aria_nbv/vin/experimental/model.py`):
...
- `candidate_poses_world_cam: PoseTW` (world←camera) and `reference_pose_world_rig: PoseTW` (world←rig_reference)
  from `OracleRriLabelBatch.depths` (used in `aria_nbv/aria_nbv/lightning/lit_datamodule.py`).
...
- [x] Implement `aria_nbv/aria_nbv/vin/backbone_evl.py` as a clean adapter:
...
- [x] Implement the shell-aware pose descriptor (`r, u, f, scalars`) in the reference rig frame.
  ...
  > **Done**: Implemented in `aria_nbv/aria_nbv/vin/experimental/model.py` by consuming `reference_pose_world_rig` (world←rig) and `candidate_poses_world_cam` (world←cam), forming $T_{\mathrm{rig}\leftarrow\mathrm{cam}}$, and computing $(r, u, f, \mathrm{scalars})$ in the reference rig frame.

- [x] Implement `ShellShPoseEncoder` in `aria_nbv/aria_nbv/vin/spherical_encoding.py`:
  - uses `e3nn.o3.spherical_harmonics` for `u` and `f`
  - no `so3log`
  - outputs `E_pose` per candidate

  > **Done**: Implemented as `aria_nbv/aria_nbv/vin/spherical_encoding.py` (`ShellShPoseEncoder` + config) using `e3nn.o3.spherical_harmonics`, with **1D Fourier features** for the radius (`r` by default; optionally $\log(r+\varepsilon)$ via config).
...
- [x] Implement `VinModel` (frozen EVL + trainable head):
  ...
  > **Done**: Implemented in `aria_nbv/aria_nbv/vin/experimental/model.py` with shell+SH pose encoding, a minimal voxel field (`occ_pr`, `occ_input`, `counts_norm`), mean-pooled global context, masked-mean frustum sampling for candidates using PyTorch3D `PerspectiveCameras`, and CORAL logits/expected score outputs.

- [x] Implement `VinPrediction` dataclass (logits, scores, masks)

  > **Done**: Implemented in `aria_nbv/aria_nbv/vin/types.py`.
...
- [x] Implement an online VIN training dataset wrapper using oracle labels:
  - `aria_nbv/aria_nbv/lightning/lit_datamodule.py` (`VinOracleIterableDataset` + `VinDataModule`)
...
- [x] Add CLI entry points (installed via `aria_nbv/pyproject.toml`):
  ...
  - (dev convenience) `aria_nbv/scripts/train_vin_lightning.py`
...
- [x] Implement RRI→ordinal binning:
  ...
  > **Done**: Implemented `aria_nbv/aria_nbv/rri_metrics/rri_binning.py` (`RriOrdinalBinner.fit_from_iterable/transform/save/load`) and wired it into the Lightning CLI (`nbv-fit-binner` / `nbv-train --fit-binner-only`).
...
- [x] Apply CORAL:
  ...
  > **Done**: Implemented `aria_nbv/aria_nbv/rri_metrics/coral.py` using `coral-pytorch` and trained with `coral_loss` in `aria_nbv/aria_nbv/lightning/lit_module.py`.
