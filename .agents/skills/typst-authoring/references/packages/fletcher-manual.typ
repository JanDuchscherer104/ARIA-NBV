#import "/src/exports.typ" as fletcher: diagram, edge, node
...
#show raw.where(block: false): it => {
  ...
  if it.text.match(regex("^[a-z-]+\(\)$")) == none { it } else {
    let l = label(it.text)
    context {
      if query(l).len() > 0 {
        link(l, it)
...
#table(
  ...
  ..{
    ...
    (
      ...
      diagram(node(
        (0, 0),
        box(inset: 1pt, link(label(name + "()"), raw(name))),
        shape: shape,
  ...
  diagram(
    node((0, 0), link(label("stretched-glyph()"), `stretched-glyph`), shape: fletcher.shapes.stretched-glyph.with(
      glyph: $<->$,
    )),
  ),
  ...
  spacing: (14mm, 12mm),
  {
    for (i, str) in (
      ...
      for j in range(2) {
        ...
        edge((2 * i, j), (2 * i + 1, j), str, bend: 50deg * j, stroke: 0.9pt, label: label, label-sep: 1em)
      }
    }
  },
))
...
#align(center, fletcher.diagram(
  ...
  spacing: (14mm, 12mm),
  {
    for (i, str) in (
      ...
      let label = raw("\"" + str + "\"")
      edge((2 * i, 0), (2 * i + 1, 0), str, stroke: 0.9pt, label: label, label-sep: 1em)
    }
  },
))
