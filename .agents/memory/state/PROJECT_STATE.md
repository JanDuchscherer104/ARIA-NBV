...
- *Current truth / active direction:* The thesis plan now has a hard gated core beyond one-step scoring: V0/V1 target contracts, observed-only target selection, mixed observed candidate sets, stable ASE oracle rollouts, and target-conditioned fitted Double-Q / Q_H over finite candidate sets.
- *Current truth / active direction:* The learned VIN remains a preliminary one-step candidate scorer rather than a full next-best-view policy. The required M5 policy-like result is Q_H trained from ASE oracle rollout data; full continuous control, online RL, and real-device deployment remain bridge/future work.
...
- *Current truth / active direction:* Use discrete search, stochastic rollouts, and fitted Double-Q / Q_H as the first non-myopic learning milestone before attempting full continuous control.
- *Current truth / active direction:* Make bounded oracle-RRI lookahead versus one-step greedy under equal budget the first non-myopic comparison, then train Q_H from ASE oracle rollout data and evaluate it against one-step greedy/model scoring.
...
- *Current truth / active direction:* Scale within the current ecosystem before switching worlds: use small trusted subsets for correctness, then target the full 100 GT-mesh scenes / 4,608 snippet windows with scene-level splits and exact coverage reporting.
