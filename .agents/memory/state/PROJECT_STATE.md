...
6. *Evidence bar for RL / planning claims.* *Status:* Locked for the first VIN gate. *Current state:* the paper still positions full policy learning as future work, and current rollout / RL results are scaffolding rather than thesis-grade evidence. *Why it matters now:* it defines what has to be measured before making stronger claims. *Decision:* require held-out ranking, oracle-evaluated VIN-selected rollouts, calibration / stage-shift analysis, and Rerun failure visualization before adding value, Q, or return heads.
7. *MDP contract and modality split.* *Status:* Locked for the first rollout actor. *Current state:* full modalities are only available on logged ego-trajectories, while counterfactual states currently rely on geometry-derived quantities and accumulated selected observations. *Why it matters now:* this is the central design decision for multi-step learning. *Decision:* keep the first actor geometry-first; oracle labels, GT mesh quantities, and GT OBB crops are supervision/evaluation only unless a later ablation explicitly marks them privileged.
...
- *Current truth / active direction:* Make bounded oracle-RRI lookahead versus one-step greedy under equal budget the first non-myopic comparison, then test whether VIN can become a fast surrogate reward or critic.
...
- *Current truth / active direction:* Use GT-OBB-cropped target RRI as the first target-aware metric before predicted-OBB or learned target-token realism ablations.
